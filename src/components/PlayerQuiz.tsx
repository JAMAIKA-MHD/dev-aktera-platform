...
import { CheckCircle2, XCircle, HelpCircle, ChevronRight, Trophy, Frown } from 'lucide-react';
          ...
          {passed ? (
            <>
              ...
              <p className="text-sm text-zinc-400 max-w-xs">
                You got <span className="text-emerald-400 font-bold">{correctCount} out of {total}</span> correct.
                <br />Time to spin the wheel!
              </p>
            ...
            <>
              ...
              <p className="text-sm text-zinc-400 max-w-xs">
                You got <span className="text-red-400 font-bold">{correctCount} out of {total}</span> correct.
                <br />You need {passThreshold} to win — let's spin anyway!
              </p>
              <p dir="auto" className="text-xs text-zinc-500 mt-1">
    ...
    <div className="flex flex-col min-h-full bg-[#0F0F1A]">
      ...
      <div className="px-5 pt-6 pb-4 bg-[#1E1E2E] border-b border-white/5">
        ...
        <p className="mt-2 text-[11px] text-zinc-500">
          Hey <span className="text-zinc-300 font-semibold">{playerName}</span> — answer to unlock the wheel!
        </p>
      </div>
          ...
          >
            ...
            <div className="bg-[#1E1E2E] rounded-2xl p-5 border border-white/5 shadow-md">
              <div className="flex items-start gap-3">
                <HelpCircle className="w-5 h-5 mt-0.5 flex-shrink-0" style={{ color: activeBrand.primaryColor }} />
                <p dir="auto" className="text-sm font-bold text-white leading-relaxed">
              ...
              {current.options.map((opt, idx) => {
                ...
                if (answerState !== 'idle' && selectedOption !== null) {
                  if (idx === current.correctIndex) {
                    ...
                    optionClass = 'bg-emerald-950/80 border-emerald-500/50 text-emerald-200';
                  } else if (idx === selectedOption && answerState === 'wrong') {
                    // Highlight wrong selection in red
                    optionClass = 'bg-red-950/80 border-red-500/50 text-red-300';
                  } else {
                    optionClass = 'bg-[#1E1E2E] border-white/5 text-zinc-500';
                  }
                }

                const isCorrectAnswer = answerState !== 'idle' && idx === current.correctIndex;
                const isWrongAnswer = answerState === 'wrong' && idx === selectedOption;
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl border text-left font-semibold text-sm transition-colors duration-200 min-h-[52px] cursor-pointer ${optionClass}`}
                  >
                    <span dir="auto" className="flex-1">{opt}</span>
                    {isCorrectAnswer && <CheckCircle2 className="w-5 h-5 text-emerald-400 flex-shrink-0" />}
                    {isWrongAnswer && <XCircle className="w-5 h-5 text-red-400 flex-shrink-0" />}
                  </motion.button>
                );
              })}
            </div>
            ...
            <AnimatePresence>
              {answerState !== 'idle' && (
                ...
                >
                  {answerState === 'correct' ? '✅ Correct! Next question...' : '❌ Wrong answer!'}
                </motion.p>
              )}
            </AnimatePresence>
