...
const PLAN_PRICES: Record<BillingRecord['plan'], { price: string; period: string; quota: number; features: string[] }> = {
  free: {
    price: 'Free',
...
export const BillingUsage: React.FC = () => {
    ...
    const fetchBilling = async () => {
      try {
        const { data, error: queryError } = await supabase
          .from('billing')
          .select('id, plan, billing_cycle, amount, status, period_start, period_end')
  ...
  const usagePercent = Math.min(100, currentPlan.quota > 0 ? (totalEntries / currentPlan.quota) * 100 : 0);
  const paidAmount = useMemo(
    () => records.filter((record) => record.status === 'paid').reduce((sum, record) => sum + Number(record.amount), 0),
    [records],
  );
  const pendingAmount = useMemo(
    () => records.filter((record) => record.status === 'pending').reduce((sum, record) => sum + Number(record.amount), 0),
    [records],
  );
    ...
    <div id="billing-usage-root" className="space-y-6 text-slate-800">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">Track live usage against the current plan and review billing history returned by Supabase.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        {[
          { title: 'Current plan', value: PLAN_LABELS[currentPlanKey], desc: currentPlan.period, icon: CreditCard, color: 'text-indigo-600' },
          { title: 'Paid amount', value: `${paidAmount.toLocaleString()} DZD`, desc: 'Settled invoice total', icon: Receipt, color: 'text-emerald-600' },
          { title: 'Pending amount', value: `${pendingAmount.toLocaleString()} DZD`, desc: 'Awaiting payment collection', icon: AlertTriangle, color: 'text-amber-600' },
          { title: 'Invoice rows', value: records.length, desc: 'Returned by the billing table', icon: ShieldCheck, color: 'text-sky-600' },
        ].map((stat) => (
          <div key={stat.title} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">{stat.title}</span>
              <stat.icon className={`w-4 h-4 ${stat.color}`} />
            </div>
            <h4 className="text-2xl font-extrabold mt-3 mb-1 text-slate-900">{stat.value}</h4>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        <div className="space-y-3.5">
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="text-indigo-600 font-bold font-mono">
              {totalEntries.toLocaleString()} / {currentPlan.quota.toLocaleString()} Captures
            </span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden border border-gray-200">
            <div className="bg-gradient-to-r from-indigo-600 to-indigo-400 h-full rounded-full" style={{ width: `${usagePercent}%` }} />
          </div>
          <p className={`text-[10px] flex items-center gap-1.5 leading-normal ${usagePercent >= 90 ? 'text-amber-600' : 'text-emerald-600'}`}>
            <AlertTriangle className="w-3.5 h-3.5" />
            <span>{usagePercent >= 90 ? 'Nearing limit. Upgrade planning should happen soon.' : 'Usage is still within the current plan allowance.'}</span>
          </p>
        </div>
        ...
        <div className="space-y-1.5 text-xs text-slate-500 pl-0 md:pl-6 border-t md:border-t-0 md:border-l border-gray-200 pt-4 md:pt-0">
          ...
          <p>• <strong>Active portals:</strong> {activeCampaigns} live url(s)</p>
          <p>• <strong>Current tier:</strong> {PLAN_LABELS[currentPlanKey]}</p>
          <p>• <strong>Invoice cycle resets:</strong> {formatDate(records[0]?.period_end)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">Live rows from the `billing` table for this organization.</p>
            </div>
          </div>
                  ...
                  {records.map((record) => {
                    ...
                    return (
                      <tr key={record.id} className="hover:bg-slate-50/60">
                        <td className="py-3 pl-1 font-semibold text-slate-800">{PLAN_LABELS[record.plan]}</td>
        ...
        <div className="grid grid-cols-1 gap-6">
          {(Object.keys(PLAN_LABELS) as BillingRecord['plan'][]).map((planKey) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <span className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">{PLAN_LABELS[planKey]}</span>
                  <div className="flex items-baseline gap-1 mt-2">
                    <span className="text-3xl font-extrabold text-slate-900">{pricing.price}</span>
                    <span className="text-xs text-slate-400 font-mono">{pricing.period}</span>
                  </div>
