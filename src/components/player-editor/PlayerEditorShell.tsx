...
import { ArrowLeft, Monitor, Tablet, Smartphone, Undo2, Redo2, ChevronDown } from "lucide-react";
...
const DEFAULT_CONFIG: PlayerScreenConfig = {
  ...
  content: {
    preGame: { title: "Win Big!", subHeader: "Enter your details to spin the wheel", rulesText: "Terms apply.", formFields: [] },
    winState: { title: "You Won!", ctaLabel: "Claim Now", ctaAction: "claim" },
    loseState: { title: "Better luck next time", ctaLabel: "Try Again", ctaAction: "retry" },
...
function mergeConfig(dbConfig?: any): PlayerScreenConfig {
  ...
  return {
    theme: { 
      ...
      ...(parsed.theme || {}),
      background: { ...DEFAULT_CONFIG.theme.background, ...(parsed.theme?.background || {}) }
    },
    gameAssets: { 
      ...DEFAULT_CONFIG.gameAssets, 
      ...(parsed.gameAssets || {}),
      sound: { ...DEFAULT_CONFIG.gameAssets.sound, ...(parsed.gameAssets?.sound || {}) }
    },
    content: { 
      ...DEFAULT_CONFIG.content, 
      ...(parsed.content || {}),
      preGame: { ...DEFAULT_CONFIG.content.preGame, ...(parsed.content?.preGame || {}) },
      winState: { ...DEFAULT_CONFIG.content.winState, ...(parsed.content?.winState || {}) },
      loseState: { ...DEFAULT_CONFIG.content.loseState, ...(parsed.content?.loseState || {}) },
      gameParams: { ...DEFAULT_CONFIG.content.gameParams, ...(parsed.content?.gameParams || {}) },
    }
  };
}

export function PlayerEditorShell({ campaigns, selectedCampaignId, onSelectCampaign, onClose, onSave }: PlayerEditorShellProps) {
  const campaign = campaigns.find(c => c.id === selectedCampaignId) || campaigns[0];
  
  // Initialize with existing config or default
  const [config, setConfig] = React.useState<PlayerScreenConfig>(mergeConfig(campaign?.playerScreenConfig));
  ...
  if (!campaign) {
    return (
      <div className="fixed inset-0 z-50 bg-brand-dark flex flex-col items-center justify-center text-brand-text">
        <p className="text-brand-text-muted">No campaigns available. Create one first.</p>
        <button onClick={onClose} className="mt-4 px-4 py-2 glass-panel border border-brand-border rounded transition-colors hover:bg-brand-surface/50">Go Back</button>
      </div>
    );
  }
      ...
      <header className="h-16 glass-panel border-b border-brand-border flex items-center justify-between px-4 shrink-0 text-brand-text">
              ...
              return (
                <button
                  ...
                  className={`p-1.5 rounded-md transition-colors ${
                    isActive ? "bg-brand-primary text-white shadow" : "text-brand-text-muted hover:text-brand-text"
                  }`}
                >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 mr-2 border-r border-brand-border pr-4">
            <button className="p-1.5 text-brand-text-muted hover:text-brand-text transition-colors" title="Undo"><Undo2 className="w-4 h-4" /></button>
            <button className="p-1.5 text-brand-text-muted hover:text-brand-text transition-colors" title="Redo"><Redo2 className="w-4 h-4" /></button>
          </div>
