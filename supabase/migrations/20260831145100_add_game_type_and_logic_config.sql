-- Migration: add_game_type_and_logic_config
-- Supports the 5 game types (wheel, quiz, scratch, mystery box, hit it) and dynamic server-side resolution configs.

-- 1. Add new columns to campaigns
ALTER TABLE public.campaigns
ADD COLUMN IF NOT EXISTS game_type text NOT NULL DEFAULT 'lucky_wheel'
  CHECK (game_type IN ('lucky_wheel', 'quiz', 'scratch_card', 'mystery_box', 'hit_it'));

ALTER TABLE public.campaigns
ADD COLUMN IF NOT EXISTS game_logic_config jsonb NOT NULL DEFAULT '{}'::jsonb;

-- Migrate existing rows if require_quiz is true
UPDATE public.campaigns
SET game_type = 'quiz'
WHERE require_quiz = true;

-- 2. Update save_campaign_full_in_place RPC with game_type and game_logic_config support
DROP FUNCTION IF EXISTS public.save_campaign_full_in_place(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, INTEGER, BOOLEAN, JSONB, JSONB);
DROP FUNCTION IF EXISTS public.save_campaign_full_in_place(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, INTEGER, BOOLEAN, JSONB, JSONB, BOOLEAN);
DROP FUNCTION IF EXISTS public.save_campaign_full_in_place(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, INTEGER, BOOLEAN, JSONB, JSONB, BOOLEAN, JSONB);
-- We should also drop the old 17-parameter signature to replace it with a 19-parameter one.
DROP FUNCTION IF EXISTS public.save_campaign_full_in_place(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, INTEGER, BOOLEAN, JSONB, JSONB, BOOLEAN, JSONB);
...
CREATE OR REPLACE FUNCTION save_campaign_full_in_place(
  ...
  p_player_screen_config JSONB DEFAULT NULL,
  p_game_type TEXT DEFAULT 'lucky_wheel',
  p_game_logic_config JSONB DEFAULT '{}'::jsonb
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_campaign_id IS NOT NULL AND EXISTS (SELECT 1 FROM campaigns WHERE id = p_campaign_id) THEN
    ...
    SET
      ...
      player_screen_config = p_player_screen_config,
      game_type = p_game_type,
      game_logic_config = p_game_logic_config,
      updated_at = now()
    WHERE id = p_campaign_id;
...
GRANT EXECUTE ON FUNCTION public.save_campaign_full_in_place(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, INTEGER, BOOLEAN, JSONB, JSONB, BOOLEAN, JSONB, TEXT, JSONB) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.draw_and_claim_campaign_prize(UUID, BOOLEAN) TO anon, authenticated, service_role;
